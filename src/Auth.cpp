...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/17 11:28:02 by edubois-          #+#    #+#             */
/*   Updated: 2025/11/17 11:31:06 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::clientAuth(int fd, std::string cmd)
{
	...
	else if(!cli->getRegistered())
	{
        ...
        _sendResponse(ERR_ALREADYREGISTERED(GetServerClient(fd)->GetNickName()), fd);
}
	...
	else
	{
		if(cli && cli->getRegistered())
		{
			...
			for(size_t i = 0; i < channels.size(); i++){
				Client *cl = channels[i].GetServerClientInChannel(oldnick);
				if(cl)
					cl->SetNickname(cmd);
			}
