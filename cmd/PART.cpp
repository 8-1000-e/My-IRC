...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/17 13:04:21 by edubois-          #+#    #+#             */
/*   Updated: 2025/11/17 13:04:22 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::PART(std::string cmd, int fd)
{
	...
	for (size_t i = 0; i < tmp.size(); i++){
		...
		for (size_t j = 0; j < this->channels.size(); j++){ // search for the channel
			if (this->channels[j].GetServerName() == tmp[i]){ // check if the channel exist
				flag = true;
				if (!channels[j].GetChannelClient(fd) && !channels[j].get_admin(fd)) // ERR_NOTONCHANNEL (442) // if the client is not in the channel
					{senderror(442, GetServerClient(fd)->GetNickName(), "#" + tmp[i], GetServerClient(fd)->GetFd(), " :You're not on that channel\r\n"); continue;}
					std::stringstream ss;
					ss << ":" << GetServerClient(fd)->GetNickName() << "!~" << GetServerClient(fd)->GetUserName() << "@" << "localhost" << " PART #" << tmp[i];
					if (!reason.empty())
						ss << " :" << reason << "\r\n";
					else ss << "\r\n";
					channels[j].sendToAll(ss.str());
					if (channels[j].get_admin(channels[j].GetServerClientInChannel(GetServerClient(fd)->GetNickName())->GetFd()))
						channels[j].remove_admin(channels[j].GetServerClientInChannel(GetServerClient(fd)->GetNickName())->GetFd());
					else
						channels[j].remove_client(channels[j].GetServerClientInChannel(GetServerClient(fd)->GetNickName())->GetFd());
					if (channels[j].GetServerClientsNumber() == 0)
