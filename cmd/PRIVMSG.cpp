...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/17 13:04:23 by edubois-          #+#    #+#             */
/*   Updated: 2025/11/17 13:04:24 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	for(size_t i = 0; i < tmp.size(); i++){
		if (tmp[i][0] == '#'){
			...
			if(!GetChannel(tmp[i]))//ERR_NOSUCHNICK (401) // if the channel doesn't exist
				{senderror(401, "#" + tmp[i], GetServerClient(fd)->GetFd(), " :No such nick/channel\r\n"); tmp.erase(tmp.begin() + i); i--;}
			else if (!GetChannel(tmp[i])->GetServerClientInChannel(GetServerClient(fd)->GetNickName())) //ERR_CANNOTSENDTOCHAN (404) // if the client is not in the channel
				{senderror(404, GetServerClient(fd)->GetNickName(), "#" + tmp[i], GetServerClient(fd)->GetFd(), " :Cannot send to channel\r\n"); tmp.erase(tmp.begin() + i); i--;}
			else tmp[i] = "#" + tmp[i];
		}
		else{
			if (!GetServerClientNick(tmp[i]))//ERR_NOSUCHNICK (401) // if the client doesn't exist
				{senderror(401, tmp[i], GetServerClient(fd)->GetFd(), " :No such nick/channel\r\n"); tmp.erase(tmp.begin() + i); i--;}
		}
	}
}
...
void	Server::PRIVMSG(std::string cmd, int fd)
{
	...
	if (!tmp.size())//ERR_NORECIPIENT (411) // if the client doesn't specify the recipient
		{senderror(411, GetServerClient(fd)->GetNickName(), GetServerClient(fd)->GetFd(), " :No recipient given (PRIVMSG)\r\n"); return;}
	if (message.empty())//ERR_NOTEXTTOSEND (412) // if the client doesn't specify the message
		{senderror(412, GetServerClient(fd)->GetNickName(), GetServerClient(fd)->GetFd(), " :No text to send\r\n"); return;}
	if (tmp.size() > 10) //ERR_TOOMANYTARGETS (407) // if the client send the message to more than 10 clients
		{senderror(407, GetServerClient(fd)->GetNickName(), GetServerClient(fd)->GetFd(), " :Too many recipients\r\n"); return;}
	...
	for (size_t i = 0; i < tmp.size(); i++){// send the message to the clients and channels
		if (tmp[i][0] == '#'){
			tmp[i].erase(tmp[i].begin());
			std::string resp = ":" + GetServerClient(fd)->GetNickName() + "!~" + GetServerClient(fd)->GetUserName() + "@localhost PRIVMSG #" + tmp[i] + " :" + message + "\r\n";
			GetChannel(tmp[i])->sendToAll(resp, fd);
		}
		else{
			std::string resp = ":" + GetServerClient(fd)->GetNickName() + "!~" + GetServerClient(fd)->GetUserName() + "@localhost PRIVMSG " + tmp[i] + " :" + message + "\r\n";
			_sendResponse(resp, GetServerClientNick(tmp[i])->GetFd());
		}
	}
}
