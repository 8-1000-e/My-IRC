...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/17 13:04:19 by edubois-          #+#    #+#             */
/*   Updated: 2025/11/17 13:04:20 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::mode_command(std::string& cmd, int fd)
{
	...
	else if (!channel->GetChannelClient(fd) && !channel->get_admin(fd))
	{
		senderror(442, GetServerClient(fd)->GetNickName(), channelName, GetServerClient(fd)->GetFd(), " :You're not on that channel\r\n"); return;
	}
	else if (modeset.empty()) // response with the channel modes (MODE #channel)
	{
		_sendResponse(RPL_CHANNELMODES(cli->GetNickName(), channel->GetServerName(), channel->getModes()) + \
		RPL_CREATIONTIME(cli->GetNickName(), channel->GetServerName(),channel->get_creationtime()),fd);
		return ;
	}
	else if (!channel->get_admin(fd)) // client is not channel operator
	{
		_sendResponse(ERR_NOTOPERATOR(channel->GetServerName()), fd);
		return ;
	}
	else if(channel)
	{
		...
		for(size_t i = 0; i < modeset.size(); i++)
		{
			...
			else
			{
				if(modeset[i] == 'i')//invite mode
					...
					_sendResponse(ERR_UNKNOWNMODE(cli->GetNickName(), channel->GetServerName(),modeset[i]),fd);
			}
		}
	}
	...
	if(chain.empty())
		return ;
 	channel->sendToAll(RPL_CHANGEMODE(cli->getHostname(), channel->GetServerName(), mode_chain.str(), arguments));
}
...
std::string Server::password_mode(std::vector<std::string> tokens, Channel *channel, size_t &pos, char opera, int fd, std::string chain, std::string &arguments)
{
	...
	{
		_sendResponse(ERR_NEEDMODEPARM(channel->GetServerName(),std::string("(k)")),fd);
		return param;
	}
	if(!validPassword(pass))
	{
		_sendResponse(ERR_INVALIDMODEPARM(channel->GetServerName(),std::string("(k)")),fd);
		return param;
	}
	if(opera == '+')
	{
	...
	else if (opera == '-' && channel->getModeAtindex(2))
	{
		...
		{		
			...
			_sendResponse(ERR_KEYSET(channel->GetServerName()),fd);
	}
	return param;
}
...
std::string Server::operator_privilege(std::vector<std::string> tokens, Channel *channel, size_t& pos, int fd, char opera, std::string chain, std::string& arguments)
{
	...
	{
		_sendResponse(ERR_NEEDMODEPARM(channel->GetServerName(),"(o)"),fd);
		return param;
	}
	if(!channel->clientInChannel(user))
	{
		_sendResponse(ERR_NOSUCHNICK(channel->GetServerName(), user),fd);
		return param;
	}
	if(opera == '+')
	{
...
std::string Server::channel_limit(std::vector<std::string> tokens,  Channel *channel, size_t &pos, char opera, int fd, std::string chain, std::string& arguments)
{
	...
	if(opera == '+')
	{
		if(tokens.size() > pos )
		{
			...
			{
				_sendResponse(ERR_INVALIDMODEPARM(channel->GetServerName(),"(l)"), fd);
			}
			else
			{
			...
			_sendResponse(ERR_NEEDMODEPARM(channel->GetServerName(),"(l)"),fd);
	}
